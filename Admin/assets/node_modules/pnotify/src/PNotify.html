...
<div ref:elem
    ...
    on:mouseout="fire('mouseout', event)"
    on:mouseenter="fire('mouseenter', event)"
    on:mouseleave="fire('mouseleave', event)"
    on:mousemove="fire('mousemove', event)"
    on:mousedown="fire('mousedown', event)"
    on:mouseup="fire('mouseup', event)"
    on:click="fire('click', event)"
    on:dblclick="fire('dblclick', event)"
    on:focus="fire('focus', event)"
    on:blur="fire('blur', event)"
    on:touchstart="fire('touchstart', event)"
    on:touchmove="fire('touchmove', event)"
    on:touchend="fire('touchend', event)"
    on:touchcancel="fire('touchcancel', event)">
  <div ref:container
      class="
    ...
    {{/each}}
    {{#if icon !== false}}
      <div class="ui-pnotify-icon {{_styles.icon ? _styles.icon : ''}}">
        <span class="{{icon === true ? (_icons[type] ? _icons[type] : '') : icon}}"></span>
      </div>
    {{/if}}
    <h4 ref:titleContainer class="ui-pnotify-title {{_styles.title ? _styles.title : ''}}" style="display: {{title === false ? 'none' : 'block'}}">
      {{#if titleTrusted}}
        {{{title}}}
      {{else}}
        {{title}}
    ...
    <div ref:textContainer class="ui-pnotify-text {{_styles.text ? _styles.text : ''}}" style="display: {{text === false ? 'none' : 'block'}}" role="alert">
      {{#if textTrusted}}
        {{{text}}}
      {{else}}
        {{text}}
